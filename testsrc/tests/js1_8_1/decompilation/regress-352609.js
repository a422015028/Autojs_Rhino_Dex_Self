...
function test()
{
  ...
  expect = /TypeError: (p.z = \[let \(defpackage = 3, y = 4\) defpackage\]|.*Array.*) is not a function/;
  try
  {
  ...
  expect = /TypeError: (p.z = let \(defpackage\) defpackage|.*Undefined.*) is not a function/;
  try
  {
    var p = {}; (p.z = let(x) x)()
  }
